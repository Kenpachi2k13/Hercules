			   ...
			   uint32 mode, int flag, unsigned int duration)
{
	...
	for (int i = 0, j = MAX_SKILL_TREE - 1; j >= 0 && i < MAX_MOBSKILL; j--) {
		...
		if (flag == 0 && skill->get_unit_id(skill_id, sd->status.skill[idx].lv, 0, NULL, NULL) != 0
		    && (skill->get_unit_flag(skill_id, NULL, NULL) & (UF_NOMOB | UF_NOPC)) > 0) {
			continue;
		}
		...
		if ((inf & INF_ATTACK_SKILL) == INF_ATTACK_SKILL) {
			...
			if (skill->get_range(skill_id, mob_skills[i].skill_lv, NULL, NULL) > 3)
			...
			} else if (skill->get_unit_target(skill_id, sd->status.skill[idx].lv, NULL, NULL) == BCT_ENEMY) { /// Target Enemy.
				mob_skills[i].state = MSS_ANYTARGET;
				mob_skills[i].target = MST_TARGET;
				mob_skills[i].cond1 = MSC_ALWAYS;
			} else { /// Target allies.
