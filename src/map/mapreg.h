...
struct mapreg_interface {
	...
	/* */
	char num_db[32]; ///< Name of SQL table which holds global integer variables.
	char str_db[32]; ///< Name of SQL table which holds global string variables.
	/* */
	...
	void (*save_str_db) (const char *name, unsigned int index, char *value);
	void (*save) (void);
