...
static int status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int flag)
{
	...
	if (((flag & 1) != 0 && (skill->get_nk(skill_id, src, target) & NK_NO_DAMAGE) != 0 && skill_id != ALL_RESURRECTION) // Buff/debuff skills started before hiding
	    || (skill->get_ele(skill_id, 1, src, target) == ELE_EARTH && skill_id != MG_STONECURSE)) { // Ground type
...
static defType status_get_def(struct block_list *bl)
{
	...
	if (ud && ud->skilltimer != INVALID_TIMER)
		def -= def * skill->get_castdef(ud->skill_id, ud->skill_lv, bl, NULL) / 100;
