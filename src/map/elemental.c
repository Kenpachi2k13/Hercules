...
static int read_elementaldb(void)
{
	...
	while( fgets(line, sizeof(line), fp) && j < MAX_ELEMENTAL_CLASS ) {
		...
		estatus->str = cap_value(atoi(str[11]), SHRT_MIN, SHRT_MAX);
		estatus->agi = cap_value(atoi(str[12]), SHRT_MIN, SHRT_MAX);
		estatus->vit = cap_value(atoi(str[13]), SHRT_MIN, SHRT_MAX);
		estatus->int_ = cap_value(atoi(str[14]), SHRT_MIN, SHRT_MAX);
		estatus->dex = cap_value(atoi(str[15]), SHRT_MIN, SHRT_MAX);
		estatus->luk = cap_value(atoi(str[16]), SHRT_MIN, SHRT_MAX);
